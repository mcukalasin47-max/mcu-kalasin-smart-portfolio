...
export default async function handler(req,res){
 ...
 try{
  ...
  else if(body.action==='adminAssignReview')result=await adminAssignReview(body.token,body.payload||{});
  else if(body.action==='adminCreateRound')result=await adminCreateRound(body.token,body.payload||{});
  else if(body.action==='adminSetRoundStatus')result=await adminSetRoundStatus(body.token,body.payload||{});
  else if(body.action==='workTimeline')result=await workTimeline(body.token,body.payload||{});
  else if(body.action==='notifications')result=await notifications(body.token);
...
async function createWork(token,payload){
 ...
 await appendValues('Works!A:L',[[workId,session.personId,name,sanitize(payload.category,'category'),payload.startDate||'',payload.endDate||'',sanitize(payload.description,'description'),sanitize(payload.round,'round'),'DRAFT',payload.visibility==='PUBLIC'?'PUBLIC':'PRIVATE',now,now]]);
...
async function setWorkState(token,payload){
 ...
 if(['APPROVED','REVISION'].includes(action)&&!isAdmin(session))throw new PublicError('ผลรับรองต้องบันทึกผ่านกรรมการหรือผู้ดูแลระบบ',403);
 if(action==='SUBMITTED'){
  if(!String(work['รายละเอียด/ผลลัพธ์']||'').trim())throw new PublicError('กรุณากรอกรายละเอียดหรือผลลัพธ์ก่อนส่งตรวจ',400);
  const evidence=(await table('Evidence')).filter(e=>e.workId===work.workId&&String(e['สถานะ']).toUpperCase()!=='TRASH');
  if(!evidence.length)throw new PublicError('กรุณาเพิ่มหลักฐานอย่างน้อย 1 รายการก่อนส่งตรวจ',400);
 }
...
async function uploadEvidence(token,payload){
 ...
 const works=await table('Works'),work=works.find(w=>w.workId===String(payload.workId||''));if(!work)throw new PublicError('ไม่พบผลงาน',404);if(!isAdmin(session)&&work.personId!==session.personId)throw new PublicError('ไม่มีสิทธิ์เพิ่มหลักฐานในผลงานนี้',403);
 const person=(await table('Personnel')).find(p=>p.personId===work.personId);if(!person)throw new PublicError('ไม่พบข้อมูลเจ้าของผลงาน',404);
 const driveToken=String(payload.driveAccessToken||'');if(!driveToken)throw new PublicError('กรุณาอนุญาตสิทธิ์ Google Drive ก่อนอัปโหลด',401);await verifyDriveUser(driveToken,session.email);
 const evidenceRoot=await driveEnsureFolder(driveToken,process.env.DRIVE_FOLDER_ID,'02 Evidence Files'),personFolder=await driveEnsureFolder(driveToken,evidenceRoot.id,driveFolderName(`${work.personId} - ${person['ชื่อ-ฉายา/นามสกุล']||'ไม่ระบุชื่อ'}`)),categoryFolder=await driveEnsureFolder(driveToken,personFolder.id,driveCategoryName(work['หมวดผลงาน']||work.category)),workFolder=await driveEnsureFolder(driveToken,categoryFolder.id,driveFolderName(`${work.workId} - ${work['ชื่อผลงาน']||work.title||'ผลงาน'}`));
 const original=sanitize(payload.fileName,'fileName')||'evidence',fileName=driveFileName(work.workId,original),file=await driveCreate(fileName,m[1],bytes,driveToken,workFolder.id),home=await addEvidenceLink(token,{workId:payload.workId,name:payload.name||original,url:file.webViewLink,description:payload.description});await audit(session.email,work.personId,'UPLOAD_DRIVE','fileId','',file.id,'SUCCESS',`${personFolder.name}/${categoryFolder.name}/${workFolder.name}/${file.name}`);return home;
}
async function reviewerQueue(token){const session=verifySession(token);if(!isReviewer(session)&&!isAdmin(session))throw new PublicError('สงวนสิทธิ์สำหรับกรรมการและผู้ดูแล',403);const[allReviews,people,works,evidence,users,rounds]=await Promise.all([table('Reviews'),table('Personnel'),table('Works'),table('Evidence'),table('Users'),table('EvaluationRounds')]),reviews=isAdmin(session)?allReviews:allReviews.filter(r=>String(r.reviewerEmail||r['อีเมลกรรมการ']||'').toLowerCase()===session.email),submitted=works.filter(w=>['SUBMITTED','รอตรวจ','REVISION','APPROVED'].includes(String(w.status||w['สถานะ']).toUpperCase()));return{ok:true,isAdmin:isAdmin(session),reviews,rounds,reviewers:users.filter(u=>['REVIEWER','ADMIN','SUPER_ADMIN'].includes(String(u.role).toUpperCase())&&String(u.status).toUpperCase()==='ACTIVE').map(u=>({email:u.email,role:u.role,personId:u.personId})),people:people.map(p=>({personId:p.personId,name:p['ชื่อ-ฉายา/นามสกุล'],position:p['ตำแหน่ง']})),works:submitted,evidence:evidence.filter(e=>submitted.some(w=>w.workId===e.workId)&&String(e['สถานะ']).toUpperCase()!=='TRASH')}}
async function saveReview(token,payload){const session=verifySession(token);if(!isReviewer(session)&&!isAdmin(session))throw new PublicError('ไม่มีสิทธิ์บันทึกผลประเมิน',403);const reviews=await table('Reviews'),row=reviews.find(r=>r.reviewId===payload.reviewId);if(!row)throw new PublicError('ไม่พบรายการประเมิน',404);const assigned=String(row.reviewerEmail||row['อีเมลกรรมการ']||'').toLowerCase();if(!isAdmin(session)&&assigned!==session.email)throw new PublicError('รายการนี้ไม่ได้มอบหมายให้บัญชีของคุณ',403);const status=String(payload.status||'กำลังประเมิน'),score=String(payload.score||''),comment=sanitize(payload.comment,'comment');if(score&&(Number(score)<0||Number(score)>100))throw new PublicError('คะแนนต้องอยู่ระหว่าง 0–100',400);if(status==='ประเมินแล้ว'&&score==='')throw new PublicError('กรุณาระบุคะแนนก่อนรับรองผล',400);if(status==='ขอแก้ไข'&&!comment)throw new PublicError('กรุณาระบุข้อเสนอแนะที่ต้องแก้ไข',400);const values={สถานะ:status,คะแนนรวม:score,ข้อเสนอแนะ:comment,'วันที่ประเมิน':new Date().toISOString()};for(const[k,v]of Object.entries(values)){const c=row.__headers.indexOf(k)+1;if(c>0)await updateCell('Reviews',row.__row,c,v)}const targetState=status==='ขอแก้ไข'?'REVISION':status==='ประเมินแล้ว'?'APPROVED':'';if(targetState){const works=(await table('Works')).filter(w=>w.personId===row.personId&&(!row.roundId||w['รอบประเมิน']===row.roundId)&&['SUBMITTED','รอตรวจ'].includes(String(w['สถานะ'])));for(const work of works){const c=work.__headers.indexOf('สถานะ')+1,u=work.__headers.indexOf('updatedAt')+1;if(c>0)await updateCell('Works',work.__row,c,targetState);if(u>0)await updateCell('Works',work.__row,u,new Date().toISOString());await audit(session.email,work.personId,'REVIEW_WORK','workId',work['สถานะ'],targetState,'SUCCESS',`${work.workId}: ${comment}`)}}await audit(session.email,row.personId,'SAVE_REVIEW','reviewId','',row.reviewId,'SUCCESS',status);return reviewerQueue(token)}

async function adminCreateRound(token,payload){const session=verifySession(token);requireAdmin(session);const name=sanitize(payload.name,'name');if(!name)throw new PublicError('กรุณาระบุชื่อรอบประเมิน',400);const rounds=await table('EvaluationRounds'),roundId=`R${new Date().getFullYear()}-${String(rounds.length+1).padStart(3,'0')}`;await appendRecord('EvaluationRounds',{roundId,'ชื่อรอบประเมิน':name,'ประเภท':sanitize(payload.type,'type')||'ประจำปี','วันเปิดรอบ':sanitize(payload.openDate,'openDate'),'วันปิดส่งหลักฐาน':sanitize(payload.deadline,'deadline'),'วันประเมิน':sanitize(payload.reviewDate,'reviewDate'),'สถานะ':'ACTIVE'});await audit(session.email,session.personId,'CREATE_ROUND','roundId','',roundId,'SUCCESS',name);return portfolioHome(token)}
async function adminSetRoundStatus(token,payload){const session=verifySession(token);requireAdmin(session);const status=String(payload.status||'').toUpperCase();if(!['DRAFT','ACTIVE','CLOSED','ARCHIVED'].includes(status))throw new PublicError('สถานะรอบประเมินไม่ถูกต้อง',400);const rows=await table('EvaluationRounds'),row=rows.find(r=>r.roundId===payload.roundId);if(!row)throw new PublicError('ไม่พบรอบประเมิน',404);const col=row.__headers.indexOf('สถานะ')+1;if(!col)throw new PublicError('ตาราง EvaluationRounds ไม่มีคอลัมน์สถานะ',500);await updateCell('EvaluationRounds',row.__row,col,status);await audit(session.email,session.personId,'SET_ROUND_STATUS','สถานะ',row['สถานะ'],status,'SUCCESS',row.roundId);return portfolioHome(token)}
async function adminAssignReview(token,payload){const session=verifySession(token);requireAdmin(session);const personId=String(payload.personId||''),roundId=String(payload.roundId||''),reviewerEmail=String(payload.reviewerEmail||'').trim().toLowerCase();if(!personId||!reviewerEmail)throw new PublicError('กรุณาเลือกบุคลากรและระบุอีเมลกรรมการ',400);const reviews=await table('Reviews');if(reviews.some(r=>r.personId===personId&&r.roundId===roundId&&String(r.reviewerEmail).toLowerCase()===reviewerEmail))throw new PublicError('มอบหมายกรรมการรายการนี้แล้ว',409);const reviewId=`REV-${crypto.randomUUID().slice(0,8).toUpperCase()}`;await appendValues('Reviews!A:I',[[reviewId,roundId,personId,reviewerEmail,'รอประเมิน','','',new Date().toISOString(),'']]);await audit(session.email,personId,'ASSIGN_REVIEWER','reviewId','',reviewId,'SUCCESS',`${roundId}/${reviewerEmail}`);return reviewerQueue(token)}
async function workTimeline(token,payload){const session=verifySession(token),workId=String(payload.workId||''),works=await table('Works'),work=works.find(w=>w.workId===workId);if(!work)throw new PublicError('ไม่พบผลงาน',404);if(!isAdmin(session)&&!isReviewer(session)&&work.personId!==session.personId)throw new PublicError('ไม่มีสิทธิ์ดูประวัติผลงานนี้',403);const rows=(await table('AuditLog')).filter(r=>r.personId===work.personId&&[r.details,r.oldValue,r.newValue,r.field].some(v=>String(v||'').includes(workId))).slice(-100).reverse();return{ok:true,work,rows}}
async function notifications(token){const session=verifySession(token),[works,reviews,audits]=await Promise.all([table('Works'),table('Reviews'),table('AuditLog')]),mine=isAdmin(session)?works:works.filter(w=>w.personId===session.personId),assigned=(isAdmin(session)?reviews:reviews.filter(r=>String(r.reviewerEmail).toLowerCase()===session.email)).filter(r=>!['ประเมินแล้ว'].includes(String(r['สถานะ']))),revision=mine.filter(w=>String(w['สถานะ']).toUpperCase()==='REVISION'),pending=mine.filter(w=>['SUBMITTED','รอตรวจ'].includes(String(w['สถานะ'])));return{ok:true,summary:{total:assigned.length+revision.length,pending:pending.length,revision:revision.length,assigned:assigned.length},items:[...revision.map(w=>({type:'REVISION',title:`ต้องแก้ไข: ${w['ชื่อผลงาน']}`,workId:w.workId})),...assigned.map(r=>({type:'REVIEW',title:`รายการประเมิน ${r.personId} · ${r.roundId}`,reviewId:r.reviewId})),...audits.filter(a=>a.personId===session.personId).slice(-10).reverse().map(a=>({type:'AUDIT',title:a.action,detail:a.details,at:a.timestamp}))].slice(0,20)}}
async function publicPortfolio(slug){const[people,works,evidence]=await Promise.all([table('Personnel'),table('Works'),table('Evidence')]),person=people.find(p=>String(p.portfolioSlug||p.personId).toLowerCase()===String(slug||'').toLowerCase());if(!person)throw new PublicError('ไม่พบ Portfolio',404);const pub=works.filter(w=>w.personId===person.personId&&String(w.visibility).toUpperCase()==='PUBLIC'&&String(w['สถานะ']).toUpperCase()==='APPROVED'),ids=new Set(pub.map(w=>w.workId));return{ok:true,profile:{name:person['ชื่อ-ฉายา/นามสกุล'],prefix:person['คำนำหน้า/สมณศักดิ์'],position:person['ตำแหน่ง'],department:person['ฝ่ายงาน'],photoUrl:person.photoUrl},works:pub.map(w=>({workId:w.workId,name:w['ชื่อผลงาน'],category:w['หมวดผลงาน'],startDate:w['วันที่เริ่ม'],endDate:w['วันที่สิ้นสุด'],description:w['รายละเอียด/ผลลัพธ์'],round:w['รอบประเมิน'],status:'APPROVED'})),evidence:evidence.filter(e=>ids.has(e.workId)&&String(e['สถานะ']).toUpperCase()!=='TRASH').map(e=>({evidenceId:e.evidenceId,workId:e.workId,name:e['ชื่อหลักฐาน'],type:e['ประเภทไฟล์'],url:e.driveUrl,description:e['คำอธิบาย']}))}}
...
async function verifyDriveUser(token,expectedEmail){const r=await fetch('https://www.googleapis.com/oauth2/v3/userinfo',{headers:{Authorization:`Bearer ${token}`}});if(!r.ok)throw new PublicError('สิทธิ์ Google Drive หมดอายุ กรุณาอนุญาตใหม่',401);const user=await r.json();if(String(user.email||'').toLowerCase()!==String(expectedEmail||'').toLowerCase())throw new PublicError('กรุณาเลือกบัญชี Google เดียวกับที่ใช้เข้าสู่ระบบ',403)}
async function driveCreate(name,mimeType,bytes,userToken='',parentId=process.env.DRIVE_FOLDER_ID){const token=userToken||await accessToken(),boundary=`mcu-${crypto.randomUUID()}`,meta=JSON.stringify({name,parents:[parentId]}),body=Buffer.concat([Buffer.from(`--${boundary}\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n${meta}\r\n--${boundary}\r\nContent-Type: ${mimeType}\r\n\r\n`),bytes,Buffer.from(`\r\n--${boundary}--`)]),r=await fetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,webViewLink',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':`multipart/related; boundary=${boundary}`},body});if(!r.ok){const detail=await r.text();console.error(`Drive upload ${r.status}: ${detail}`);if(r.status===403&&userToken)throw new PublicError('บัญชี Google นี้ยังไม่มีสิทธิ์เพิ่มไฟล์ในโฟลเดอร์กลาง กรุณาให้เจ้าของโฟลเดอร์แชร์เป็นผู้แก้ไข',403);if(r.status===403)throw new PublicError('Service Account ไม่มีโควตา Drive กรุณาอัปโหลดด้วยบัญชี Google ของผู้ใช้',403);if(r.status===404)throw new PublicError('ไม่พบโฟลเดอร์ Google Drive หรือบัญชีนี้ยังเข้าถึงโฟลเดอร์ไม่ได้',404);throw new PublicError(`อัปโหลด Google Drive ไม่สำเร็จ (รหัส ${r.status})`,502)}return r.json()}
async function driveEnsureFolder(token,parentId,name){const escaped=String(name).replace(/\\/g,'\\\\').replace(/'/g,"\\'"),q=encodeURIComponent(`mimeType='application/vnd.google-apps.folder' and name='${escaped}' and '${parentId}' in parents and trashed=false`),list=await fetch(`https://www.googleapis.com/drive/v3/files?q=${q}&pageSize=1&fields=files(id,name,webViewLink)`,{headers:{Authorization:`Bearer ${token}`}});if(!list.ok)throw new PublicError(`ค้นหาโฟลเดอร์ Google Drive ไม่สำเร็จ (รหัส ${list.status})`,502);const found=(await list.json()).files?.[0];if(found)return found;const created=await fetch('https://www.googleapis.com/drive/v3/files?fields=id,name,webViewLink',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({name,mimeType:'application/vnd.google-apps.folder',parents:[parentId]})});if(!created.ok){const detail=await created.text();console.error(`Drive folder ${created.status}: ${detail}`);throw new PublicError('สร้างโฟลเดอร์จัดหมวดหมู่ใน Google Drive ไม่สำเร็จ',created.status===403?403:502)}return created.json()}
function driveFolderName(value){return String(value||'ไม่ระบุ').replace(/[\\/:*?"<>|]/g,'-').replace(/\s+/g,' ').trim().slice(0,120)||'ไม่ระบุ'}
function driveCategoryName(value){const raw=String(value||'อื่น ๆ').trim(),map={'การจัดการเรียนการสอน':'01 การจัดการเรียนการสอน','วิจัยและวิชาการ':'02 วิจัยและวิชาการ','บริการวิชาการ':'03 บริการวิชาการ','ทำนุบำรุงพระพุทธศาสนา':'04 ทำนุบำรุงพระพุทธศาสนา','บริหารและงานประจำ':'05 บริหารและงานประจำ','พัฒนาตนเอง':'06 พัฒนาตนเอง'};return map[raw]||`99 ${driveFolderName(raw)}`}
function driveFileName(workId,original){const clean=driveFolderName(original),stamp=new Date().toISOString().slice(0,10);return `${stamp}_${driveFolderName(workId)}_${clean}`.slice(0,180)}
...
async function health(){const users=await table('Users');return{ok:true,service:'MCU Smart Portfolio API',version:'4.0.1',auth:'Vercel OIDC / Google WIF',database:'Google Sheets connected',drive:process.env.DRIVE_FOLDER_ID?'Google Drive configured':'Google Drive not configured',users:users.length,checkedAt:new Date().toISOString()}}
...
async function appendRecord(sheet,record){const headers=(await sheetGet(`${sheet}!A1:Z1`))[0]||[];if(!headers.length)throw new PublicError(`ไม่พบหัวตาราง ${sheet}`,500);await appendValues(`${sheet}!A:${columnName(headers.length)}`,[headers.map(h=>String(record[h]??''))])}
