...
async function reviewerQueue(token){const session=verifySession(token);if(!isReviewer(session)&&!isAdmin(session))throw new PublicError('สงวนสิทธิ์สำหรับกรรมการและผู้ดูแล',403);const[allReviews,people,works,evidence]=await Promise.all([table('Reviews'),table('Personnel'),table('Works'),table('Evidence')]),reviews=isAdmin(session)?allReviews:allReviews.filter(r=>String(r.reviewerEmail||r['อีเมลกรรมการ']||'').toLowerCase()===session.email),submitted=works.filter(w=>['SUBMITTED','รอตรวจ'].includes(String(w.status||w['สถานะ'])));return{ok:true,reviews,people:people.map(p=>({personId:p.personId,name:p['ชื่อ-ฉายา/นามสกุล'],position:p['ตำแหน่ง']})),works:submitted,evidence:evidence.filter(e=>submitted.some(w=>w.workId===e.workId)&&String(e['สถานะ']).toUpperCase()!=='TRASH')}}
...
async function health(){const users=await table('Users');return{ok:true,service:'MCU Smart Portfolio API',version:'3.3.0',auth:'Vercel OIDC / Google WIF',database:'Google Sheets connected',users:users.length}}
