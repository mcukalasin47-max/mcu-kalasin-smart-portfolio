...
async function health(){const users=await table('Users');return{ok:true,service:'MCU Smart Portfolio API',version:'4.0.0',auth:'Vercel OIDC / Google WIF',database:'Google Sheets connected',drive:process.env.DRIVE_FOLDER_ID?'Google Drive configured':'Google Drive not configured',users:users.length,checkedAt:new Date().toISOString()}}
